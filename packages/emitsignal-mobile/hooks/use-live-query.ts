 ...
 * data (cache, staleTime, cross-screen reuse); SSE `message` events patch that
 * same cache through `onMessage`, so live updates and navigated data stay in
 * sync. The stream URL is derived from the current query data so hooks whose
 * topics come from their own result (feed) work without extra state.
 */
export function useLiveQuery<TData>({
    ...
    useSSE({
        ...
        url: !sessionLoading && target ? target : null,
    });
